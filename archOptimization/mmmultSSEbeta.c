...
/*
void temp_first(int *temp, int* matrix, int d, int size) { //to create temporary contiguous array of first[k][d] to be turned into m128

	int i;
	
	for (i=0; i<size; i++)
		temp[i] = matrix[i*size+d];
...
int main(int argc, char  **argv)
{
  ...
  //int *temp;
  //posix_memalign((void**)&temp, 16, sizeof(int)*size);
  __m128i m1, m2,m3; 
 
/* Matrix generation proceeds normally? */  
  for (  c = 0 ; c < size ; c++ )
  	for ( d = 0 ; d < size ; d++ )    	
  ...
  clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &t1);
	...
	for ( c = 0 ; c < size ; c++ )
	{
		for ( k = 0 ; k < size ; k++ )
	    {
			...
			for (d = 0 ; d < size ; d+=4)  //this inner loop can be SSEed
			{
				edge = size - d; 
